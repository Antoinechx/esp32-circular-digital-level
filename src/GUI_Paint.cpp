...
void Paint_DrawBitMap(const unsigned char* image_buffer)
{
    ...
    for (y = 0; y < Paint.HeightByte; y++) {
        for (x = 0; x < Paint.WidthByte; x++) { // 8 pixels = 1 byte
            Addr = x + y * Paint.WidthByte;
            unsigned short pixelValue = (unsigned short)image_buffer[Addr]; // Read the pixel value
            
            // Check if the pixel value is not 0x0000
            if (pixelValue != 0x0000) {
                Paint.Image[Addr] = CYAN; // Set the color to cyan if not 0x0000
            } else {
                Paint.Image[Addr] = pixelValue; // Retain the original color if it's 0x0000
            }
        }
    }
}
